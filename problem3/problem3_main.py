"""问题 3 统一联网入口：把 ``problem3_strategy`` 的模型 ``RobotClient`` 与策略类串起来。

策略与模型全部在 ``problem3_strategy.py`` 中（同一份策略类既能接真实
``RobotClient``，也能接本地仿真器）；本文件是运行的那个：

    python problem3/problem3_main.py --check           # 只检查依赖/参数，不联网
    python problem3/problem3_main.py                    # 演练/正式统一入口

前置条件：赛方客户端已登录、已在界面选择问题3演练或正式测试、倒计时结束、接口就绪。
HTTP 无法区分演练与正式，实际场次完全由模拟器界面决定。
...
class OnlineGuardError(SystemExit):
    """联网参数安全门拒绝启动（用 SystemExit，调用方无法忽略）。"""


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="问题3统一联网入口（演练/正式由模拟器界面决定）",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=argparse.SUPPRESS,  # 兼容旧命令，已不再区分演练/正式。
    )
    parser.add_argument(
        "--case-code",
        default=None,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="结果输出目录（默认 problem3/q3_logs/run_<timestamp>）",
    )
...
def online_guard(args: argparse.Namespace) -> None:
    """在创建 HTTP 客户端之前校验队号与本机回环地址。"""
    robot_id = getattr(args, "robot_id", None)
    if not robot_id or str(robot_id).strip() in ("", "<参赛队号>"):
        raise OnlineGuardError("联网测试必须给出 --robot-id（当前登录队号）")
    base_url = getattr(args, "base_url", "") or ""
    parsed = urlparse(base_url)
    host = parsed.hostname or ""
    if parsed.scheme != "http" or not host:
        raise OnlineGuardError(f"base-url 必须是 http 回环地址，收到 {base_url!r}")
...
class OnlineResultRecorder:
    """给问题3内嵌客户端补充结果记录，不改变任何机器人动作。"""
    ...
    def _find_value(data: object, names: tuple[str, ...]) -> object | None:
        if isinstance(data, dict):
            ...
            for value in data.values():
                found = OnlineResultRecorder._find_value(value, names)
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = args.output_dir or (_HERE / "q3_logs" / f"run_{stamp}")
    out.mkdir(parents=True, exist_ok=False)
    print(f"使用队号 {args.robot_id}，模拟器 {args.base_url}")
    print(f"输出目录：{out}")
    ...
    robot = OnlineResultRecorder(inner_robot)
