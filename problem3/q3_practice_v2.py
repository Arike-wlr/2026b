# -*- coding: utf-8 -*-
"""问题3 V2：历史约束＋两步补测前瞻＋灵活补盲：赛方模拟器演练入口。

安装依赖：python -m pip install numpy "shapely>=2.0" "requests>=2.32,<3"
修改下面 ROBOT_ID 为登录参赛队号，然后运行本文件。
或运行：python q3_practice_v2.py --robot-id 你们的参赛队号
只检查依赖、不联网：python q3_practice_v2.py --check
必须在赛方客户端选择「问题3演练测试」，等待接口就绪。
HTTP 无法识别演练/正式模式；请勿选择正式测试。
"""

# 与附件2示例一样，在这里填写当前登录参赛队号。
ROBOT_ID = "202610038037"
BASE_URL = "http://127.0.0.1:2026"
POLICY = "history_regions"
...
''')
    ...
    _embedded_module('problem3_strategy', r'''"""问题 3：机器狗自动搜索、定位与清除策略。
...
from problem3_strategy import (Problem3Strategy, StrategyConfig, STATUS_UNKNOWN,
...
from problem3_strategy import STATUS_DETECTED, STATUS_UNKNOWN
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="问题3演练测试；不会选择或启动平台正式测试")
    parser.add_argument("--robot-id", default=ROBOT_ID, help="当前登录参赛队号")
    parser.add_argument("--base-url", default=BASE_URL)
    parser.add_argument("--check", action="store_true", help="只检查依赖和代码，不发送HTTP")
    parser.add_argument("--verbose", action="store_true", help="打印逐条动作")
    args = parser.parse_args(argv)
    try:
        RobotClient, CoverageStrategy, MapConfig, StrategyConfig = load_implementation()
    except ImportError as error:
        print("缺少依赖：", error)
        print('请运行：python -m pip install numpy "shapely>=2.0" "requests>=2.32,<3"')
        return 2
    if args.check:
        import numpy, shapely, requests
        print("检查通过；没有发送任何HTTP请求。")
        print("策略：", POLICY)
        print("numpy", numpy.__version__, "shapely", shapely.__version__, "requests", requests.__version__)
        return 0
    if not args.robot_id or args.robot_id.strip() == "<参赛队号>":
        parser.error("请填写文件顶部 ROBOT_ID，或传入 --robot-id 你们的参赛队号；尚未联网")
    ...
    if (url.scheme != "http" or url.hostname not in ("127.0.0.1", "localhost")
        ...
        parser.error("仅允许本机回环 HTTP 地址；尚未联网")
    print("请确认赛方客户端已选择【问题3演练测试】，倒计时结束、机器狗接口已就绪。")
    print("HTTP接口无法识别演练或正式模式；请勿选择正式测试。")
    try:
        confirmation = input("输入 问题3演练测试 后回车开始，其他输入取消：").strip()
    except (EOFError, KeyboardInterrupt):
        print("取消；没有发送HTTP请求。")
        return 2
    if confirmation != "问题3演练测试":
        print("取消；没有发送HTTP请求。")
        return 2
    out = Path(__file__).resolve().parent / "q3_logs" / datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    out.mkdir(parents=True, exist_ok=False)
    result = {"all_resolved": False, "mode": "practice_manually_confirmed", "policy": POLICY}
    robot = RobotClient(args.robot_id, base_url=args.base_url.rstrip("/"), log_dir=str(out / "http"))
    try:
        cfg = StrategyConfig(bearing_error_deg=1.01, log_path=str(out / "actions.tsv"), verbose=args.verbose)
        strategy = CoverageStrategy(robot, cfg, MapConfig(history=True, full_regions=True))
        print("开始连接演练接口。日志目录：", out)
        result.update(strategy.run())
        result["policy"] = POLICY
        result["version"] = "q3_v2_20260911"
