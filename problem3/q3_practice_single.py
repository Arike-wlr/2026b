# -*- coding: utf-8 -*-
"""问题3最新覆盖地图策略：赛方模拟器演练入口。

安装依赖：python -m pip install numpy "shapely>=2.0" "requests>=2.32,<3"
修改下面 ROBOT_ID 为登录参赛队号，然后运行本文件。
或运行：python q3_practice_single.py --robot-id 你们的参赛队号
只检查依赖、不联网：python q3_practice_single.py --check
...
POLICY = "route_joint_regions_selective_approach"
...
def main(argv=None):
    ...
    try:
        ...
        strategy = CoverageStrategy(robot, cfg, MapConfig(mode=POLICY))
