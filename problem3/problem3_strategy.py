...
1. **基础策略**（本文件顶部的实体代码）—— ``Problem3Strategy`` / ``CoverageStrategy`` /
   ``StrategyConfig`` 及题面常量，与 ``problem3_local_sim``、``problem4`` 共用；
2. **模型层**（``load_implementation()`` 内嵌）—— 最终演练采用的模型（原
   ``q3_practice_v2.py`` 单文件打包）：内嵌 client / problem3_geometry /
   ``_q3_model_base`` / coverage_strategy / lookahead_strategy。其中 ``_q3_model_base``
   是模型自带的基类**旧快照**（与顶部实体代码是两份实现，因此用私有模块名，绝不覆盖
   本模块 ``problem3_strategy``，以免影响 problem4 / problem3_local_sim）；
...
# 本地测试常量直接取本模块顶部定义；LocalSimulator 在函数内惰性导入，
# 避免与 problem3_local_sim 循环导入。
