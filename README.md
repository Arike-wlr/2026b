...
├── strategy.py                ★ 主要代码：搜索与清除策略（待编写）
├── main.py                    ★ 主要代码：程序入口（待编写），串起策略与模拟器
...
└── logs/                       运行时自动生成的日志文件（robot_年月日_时分秒.txt）
```
...
6. **日志落盘**：每次运行在项目根 `logs/` 下生成独立日志文件，记录全部请求与响应
   （规范要求机器狗自行记录，模拟器界面只显示最新 1000 条）。
